...
class GUI(QtWidgets.QMainWindow):
    ...
    def __init__(self, room_6, tile_size, player, enemy):
        super().__init__()
        ...
        self.say_hey()
        self.room_arr.add_room(self.room)
        ...
        self.timer_2.timeout.connect(self.add_character_graphics_items)
        ...
        self.timer_5 = QtCore.QTimer()  # text hint timer
    ...
    def keyPressEvent(self, event):
        ...
        """ Please note any use of "we" in the comments below refer to the writer of the comments and the reader.
        This was not a collaborative project.
        """

        """Directional input is handled here, we have WASD keys. The room transition is dealt with, saving the state of the  
        current room finding and before finding and loading the next room. This is done by detecting when the player is at the edge of the current 
         room and then moves beyond it, we also have some enemy handling processes for specific rooms"""
        ...
        if event.key() == QtCore.Qt.Key_W and not self.is_paused:
            ...
            if player.get_facing() == (0, -1):
                ...
                if player.get_location() in self.outer_walls and event.key() == QtCore.Qt.Key_W:
                    ...
                    self.room = self.room_arr.get_room(self.room_arr.get_adjacent_room(self.room, player.get_facing()))
                    ...
                    if self.room.get_coordinates() != (3, 1) and self.room.get_coordinates() != (
                    0, 1) and self.room.get_coordinates() != (2, 2):
            ...
            if player.get_facing() == (0, 1):
                ...
                if player.get_location() in self.outer_walls and event.key() == QtCore.Qt.Key_S:
                    ...
                    if self.room.get_coordinates() != (3, 3) and self.room.get_coordinates !=(2,3):
            ...
            if player.get_facing() == (-1, 0):
                ...
                if player.get_location() in self.outer_walls and event.key() == QtCore.Qt.Key_A:
                    ...
                    if self.room.get_coordinates() == (0, 2):
                        self.enemies[0].set_location(3, 3, self.room)
                        self.enemies[0].set_is_alive(True)

                    elif self.room.get_coordinates() == (1, 0):
                        self.enemies[0].set_location(1, 10, self.room)
                        self.enemies[0].set_is_alive(True)
                        self.enemies[1].set_location(20, 11, self.room)
                        self.enemies[1].set_is_alive(True)
        ...
        elif event.key() == QtCore.Qt.Key_D and not self.is_paused:
            ...
            if player.get_facing() == (1, 0):
                ...
                if player.get_location() in self.outer_walls and event.key() == QtCore.Qt.Key_D:
                    ...
                    if self.room.get_coordinates() != (3, 1) and self.room.get_coordinates() != (
                            1, 1) and self.room.get_coordinates() != (1, 2) and self.room.get_coordinates() != (
                    2, 1) and self.room.get_coordinates() != (3, 2) and self.room.get_coordinates() != (1, 0):
                        self.enemies[0].set_location(10, 10, self.room)
                        self.enemies[0].set_is_alive(True)
                        self.enemies[1].set_location(11, 11, self.room)
                        self.enemies[1].set_is_alive(True)

                    elif self.room.get_coordinates() == (1, 2):
                        self.enemies[0].set_location(3, 3, self.room)
                        self.enemies[0].set_is_alive(True)
                        self.enemies[1].set_location(20, 3, self.room)
                        self.enemies[1].set_is_alive(True)

                    elif self.room.get_coordinates() == (2, 1):
                        self.enemies[0].set_location(5, 9, self.room)
                        self.enemies[0].set_is_alive(True)

                    elif self.room.get_coordinates() == (1, 1):
                        self.enemies[0].set_location(3, 3, self.room)
                        self.enemies[0].set_is_alive(True)
            ...
            ####################################### ACTION KEYS ####################################
            """
            This next section handles the action key inputs PIMOY for sword, inventory, map, open (doors, chests and signs),
            and sling, there are some loops to check tiles around the player, calls to other methods if warranted, and manipulation of player 
            statuses and container contents.   
            """
    ...
    ###################################### AUTO MOVE #############################################################
    """
    Fairly simple auto move method for enimies using random number generator, timer, and if alive clause. We also clear previous tiles here 
    otherwise the spaces read as not empty and the enemy will eventualy run out of tiles to which they can move. The if/else room coordinates 
    tile coordinate enemy position structure below blocks the movement to certain tiles in certain rooms for enimies to prevent them blocking 
    doorways and hence game progress if killed on that tile. 
    """
        ...
        if not self.is_paused:
            while i < len(characters):
                ...
                if rn2 == 0 and enemy.get_is_alive():
                    ...
                    if (enemy_pos[0], enemy_pos[1] + 1) not in self.outer_walls:
                        if self.room.get_coordinates() == (1, 1) or self.room.get_coordinates() == (
                        1, 2) or self.room.get_coordinates() == (2, 0):
                            if (enemy_pos[0], enemy_pos[1] + 1) != (4, 6) and (enemy_pos[0], enemy_pos[1] + 1) != (
                            12, 14) and (enemy_pos[0], enemy_pos[1] + 1) != (1, 5):
                    ...
                    if (enemy_pos[0] + 1, enemy_pos[1]) not in self.outer_walls:
                        if self.room.get_coordinates() == (1, 1) or self.room.get_coordinates() == (
                        1, 2) or self.room.get_coordinates() == (2, 0):
                            if (enemy_pos[0] + 1, enemy_pos[1]) != (4, 6) and (enemy_pos[0] + 1, enemy_pos[1]) != (
                            12, 14) and (enemy_pos[0] + 1, enemy_pos[1]) != (1, 5):
                    ...
                    enemy.set_facing((-1, 0))
                    if (enemy_pos[0] - 1, enemy_pos[1] + 1) not in self.outer_walls:
                        if self.room.get_coordinates() == (1, 1) or self.room.get_coordinates() == (
                        1, 2) or self.room.get_coordinates() == (2, 0):
                            if (enemy_pos[0] - 1, enemy_pos[1]) != (4, 6) and (enemy_pos[0] - 1, enemy_pos[1]) != (
                            12, 14) and (enemy_pos[0] - 1, enemy_pos[1]) != (1, 5):
    ...
    ############################ MAP ###########################################################################
    """
    Called from the Key Method above. 
    If the game is paused we create our map rectangle loop through our room array
    looping through the tiles for each room drawing an appropriately coloured rectangle (or an empty rectangle in the case 
    of an empty treasure chest) depending on the tile type. We also mark the player position with an X. All the graphical assets are 
    stored in an array. 
    If the game is not paused all the graphical components previously stored in the array are deleted from the scene. 
    """
    ...
    def map_window(self):
        ...
        if self.is_paused:
            ...
            for rooms in self.room_arr.get_array():
                for room in rooms:
                    if room != None:
    ...
    ################################ CREATE CONTAINER ###############################################################
    """
    This method was used to create the droped container when an enemy is killed.
    """
    ...
    ########################################### INVENTORY WINDOW ####################################################
    """
    Similar to the map method although less complex, we loop through the invertory of the player creating text items, storing them
    and then temoving them on the next press of the 'I' key, using the is paused flag.
    ...
    def inventory_window(self):
        ...
        if self.is_paused:
            ...
            self.inventory_text.setPlainText("INVENTORY")
            self.inventory_text.setPos(250, 95)
            self.inventory_text_title = self.inventory_text
        ...
        if self.is_paused == False:
            ...
            self.scene.removeItem(self.inventory_text_title)

    ########################################### SLING AND SLING SHOT METHODS #####################################
    """
    The sling method starts a timer and the calls the sling shot method. The slingshot method got fairly complex, I had to calculate the
    distance from the player and direction the stone was moving in. There was also a bug that was only discovered a fair time after the
    creation of the methods where if the sling was fired into an exterior open door, so the next room over, the game would crash. This was fixed
    by not allowing the stone from the sling to pass any exterior wall coordinates. We also have loop through room tiles to find if the stone 
    hits an object, enemy, or target and then apropriate response to that situation. The stone is animated with a simple lower case o that moves 
    across the screen based on the timer call to the method, the player position, and the player facing direction.
    """
    ...
    def sling_damage(self, target):
        ...
        """
        Here we loop through the tiles in the current room loading the apropriate graphic using the graphics.py(s), 
        some items need to be "gotten" such as chests and switches to check there stauses (full/empty, on/off) so the 
        apropriate graphic can be loaded.        
        """

    def add_room_graphics(self):
        ...
        for x in self.room.tiles:
            ...
            """
            Random number gentarator and timer used to select graphic items at random intervals to create animation effects for 
    ...
    ############################################ ADD TEXT AND CLOSE TEXT ##############################

    """
    Here we can add a received message to the right hand side of the screen, a timer is then started and the text is 
    then removed after a prescribed amount of time.
    ...
    ######################################## END GAME MESSAGE ###########################################################
    """
    This method is called when the player reaches "win game" tiles in the last room. 
    We have a simple second to minutes and seconds calculation. 
    A simple thank you for playing message and some simple stats are are displayed.
    """
    ...
    def win_game_message(self):
        ...
        if self.is_paused:
            ...
            self.inventory = QtCore.QRectF(100, 100, 400, 250)
            self.inventory = QtWidgets.QGraphicsRectItem(self.inventory)
            ...
            if time_min == 1:
                self.inventory_text.setPlainText(
                    "YOUR TIME WAS: {} minute and {} seconds. ".format(int(time_min), int(time_sec)))
            else:
                self.inventory_text.setPlainText(
                    "YOUR TIME WAS: {} minutes and {} seconds. ".format(int(time_min), int(time_sec)))
    ...
    ###################################### USER INSTRUCTIONS ##################################################
    """
    User instructions displayed at the bottom of the screen these are added to depending on the player's inventory
    """
    ...
    def add_instructions(self):
        ...
        if player.get_has_sling():
            self.scene.addText("Y is slingshot").setPos(500, 680)

        elif player.get_has_map() and not player.get_has_sling():
            self.scene.addText(
                "M for map").setPos(500, 665)
    ...
    ########################## SIGN DISPLAY ##############################################################
    """
    Called when a player is next to a sign tile and presses O. Message appears on the game board above the tile location
    and is removed when the player presses O again. 
    """
    ...
    """
    Here we add the character graphics for the player and the enemies, we also handle the player orientation and player attack here. 
    """
    ...
    ##################################################### ROOM CREATION ############################################
    """
    Here we create rooms, The external wall is created by the call to Room()
    ...
    def room1(self):
        ...
        room_1 = Room(self.room_size, (0, 0))

        chest_1 = Container()

        chest_1.set_location(19, 16, room_1)
        flippers_1 = Flippers("flippers")
        chest_1.set_item(flippers_1)

        chest_2 = Container()
        chest_2.set_location(3, 19, room_1)
        key_1 = Key("key")
        chest_2.set_item(key_1)
        # Creates a horizontal line of water tiles.

        for i in range(self.room_size):
            room_1.tiles[i][18].set_water()
        for i in range(self.room_size):
            if i % 2 == 0:
                room_1.tiles[i][21].set_wall()
            room_1.tiles[i][22].set_wall()

        switch = Switch()
        switch.set_location(3, 3, room_1)
        switch_2 = Switch()
        switch_2.set_location(3, 7, room_1)
        switch_3 = Switch()
        switch_3.set_location(7, 3, room_1)
        switch_4 = Switch()
        switch_4.set_location(7, 7, room_1)
        for i in range(11):
            room_1.tiles[i][10].set_wall()
            room_1.tiles[10][i].set_wall()

        room_1.tiles[5][10].set_door(True)
        room_1.tiles[5][10].set_door(False)

        room_1.tiles[12][15].set_door(True)
        for i in range(self.room_size):
            room_1.tiles[i][15].set_wall()

        room_1.tiles[self.room_size - 1][8].set_door(True)
        room_1.tiles[self.room_size - 1][8].set_door(False)

        self.room_arr.add_room(room_1)

    def room2(self):

        room_2 = Room(self.room_size, (0, 1))

        for i in range(11, self.room_size):
            for k in range(self.room_size - 1):
                room_2.tiles[k][i].set_water()
                room_2.tiles[k][i].set_water()
                room_2.tiles[k][i].set_water()

        sign = Sign("The water is too deep to swim unaided")
        room_2.tiles[5][5].set_sign(sign)

        chest = Container()
        map = Map('map')
        chest.set_location(4, 2, room_2)
        chest.set_item(map)

        room_2.tiles[self.room_size - 1][8].set_door(True)
        room_2.tiles[self.room_size - 1][8].set_door(False)
        room_2.tiles[5][self.room_size - 1].set_door(True)
        room_2.tiles[5][self.room_size - 1].set_door(False)
        self.room_arr.add_room(room_2)

    def room3(self):

        room_3 = Room(self.room_size, (0, 2))
        for i in range(self.room_size):
            room_3.tiles[8][i].set_wall()
            room_3.tiles[12][i].set_wall()
            room_3.tiles[16][i].set_wall()
            room_3.tiles[20][i].set_wall()

        room_3.tiles[8][self.room_size - 2].set_door(True)
        room_3.tiles[8][self.room_size - 2].set_door(False)
        room_3.tiles[12][2].set_door(True)
        room_3.tiles[12][2].set_door(False)
        room_3.tiles[16][self.room_size - 2].set_door(True)
        room_3.tiles[16][self.room_size - 2].set_door(False)
        room_3.tiles[20][2].set_door(True)
        room_3.tiles[20][2].set_door(False)


        switch_1 = Switch()
        switch_1.set_location(10, 12, room_3)
        switch_2 = Switch()
        switch_2.set_location(18, 12, room_3)

        room_3.tiles[self.room_size - 1][self.room_size - 3].set_door(True)

        room_3.tiles[5][0].set_door(True)
        room_3.tiles[5][0].set_door(False)
        self.room_arr.add_room(room_3)

    def room4(self):
        room_4 = Room(self.room_size, (0, 3))

        room_4.tiles[self.room_size - 1][12].set_door(True)
        room_4.tiles[self.room_size - 1][12].set_door(False)
        chest = Container()
        sling = Sling('sling')
        sign = Sign('"Try pressing ''Y''" he said.')
        room_4.tiles[19][4].set_sign(sign)
        chest.set_location(21, 2, room_4)
        ...
        while i < 23:
            k = 1
            while k < i:
                room_4.tiles[k][i].set_wall()
                k += 1
            i += 1

        self.room_arr.add_room(room_4)

    def room5(self):

        room_5 = Room(self.room_size, (1, 0))

        for i in range(1, 10):
            room_5.tiles[i][13].set_wall()
            room_5.tiles[i][9].set_wall()

        for i in range(15, self.room_size):
            room_5.tiles[i][13].set_wall()
            room_5.tiles[i][9].set_wall()

        room_5.tiles[12][self.room_size - 1].set_door(True)  # exterior door
        room_5.tiles[12][self.room_size - 1].set_door(False)
        room_5.tiles[0][8].set_door(True)  # exterior door
        room_5.tiles[0][8].set_door(False)
        room_5.tiles[self.room_size - 1][5].set_door(True)

        for i in range(10, 14):
            room_5.tiles[9][i].set_fence()
            room_5.tiles[15][i].set_fence()

        chest_2 = Container()
        coin_2 = Coin("coin", 2)
        chest_2.set_location(11, 1, room_5)
        chest_2.set_item(coin_2)

        chest_3 = Container()
        coin_3 = Coin("coin", 4)
        chest_3.set_location(12, 1, room_5)
        chest_3.set_item(coin_3)

        chest_4 = Container()
        coin_4 = Coin("coin", 3)
        chest_4.set_location(13, 1, room_5)
        chest_4.set_item(coin_4)

        room_5.tiles[10][1].set_wall()
        room_5.tiles[14][1].set_wall()
        self.room_arr.add_room(room_5)

    def room7(self):

        room_7 = Room(self.room_size, (1, 2))

        for x in range(10, 15):
            for k in range(1, 4):

                room_7.tiles[x][1].set_wall()
                room_7.tiles[9][k].set_wall()
                room_7.tiles[15][k].set_wall()
                room_7.tiles[x][2].set_lava()
                if x != 12:
                    room_7.tiles[x][3].set_lava()

        chest = Container()
        chest.set_location(12, 3, room_7)
        heat_suit_1 = HeatSuit("heat suit")

        chest.set_item(heat_suit_1)
        room_7.tiles[12][15].set_door(True)
        for i in range(self.room_size):
            room_7.tiles[i][15].set_wall()
        for i in range(15, self.room_size):
            room_7.tiles[13][i].set_wall()

        room_7.tiles[0][self.room_size - 3].set_door(True)
        room_7.tiles[0][self.room_size - 3].set_door(False)
        room_7.tiles[self.room_size - 1][20].set_door(True)
        room_7.tiles[self.room_size - 1][20].set_door(False)
        room_7.tiles[19][self.room_size - 1].set_door(True)

        chest_2 = Container()
        chest_2.set_location(20, 12, room_7)
        key_1 = Key("key")
        chest_2.set_item(key_1)
        for i in range(18, self.room_size):
            room_7.tiles[i][10].set_wall()
            room_7.tiles[i][14].set_wall()
        self.room_arr.add_room(room_7)
    ...
    def room10(self):
        ...
        self.room_arr.add_room(room_10)
    ...
    def room11(self):
        ...
        self.room_arr.add_room(room_11)
    ...
    def room12(self):
        ...
        self.room_arr.add_room(room_12)
    ...
    def room13(self):
        ...
        chest_5 = Container()
        coin_5 = Coin("coin", 10)
        chest_5.set_location(22, 18, room_13)
        chest_5.set_item(coin_5)

        self.room_arr.add_room(room_13)
    ...
    def room14(self):
        ...
        self.room_arr.add_room(room_14)
    ...
    def room15(self):
        ...
        room_15.tiles[5][8].set_sign(sign)
        ...
        self.room_arr.add_room(room_15)
    ...
    def room16(self):
        ...
        for i in range(self.room_size):
            if i % 2 == 0:
                room_16.tiles[i][20].set_wall()
            room_16.tiles[i][21].set_wall()
            room_16.tiles[i][22].set_wall()
        for i in range(10, 15):
            for k in range(21, self.room_size):
                room_16.tiles[i][k].set_door(True)
                room_16.tiles[i][k].set_door(False)
                room_16.tiles[i][21].set_fence()

        self.room_arr.add_room(room_16)

    ############################################ VARIOUS SUBSEQUENT METHODS ##########################

    """
    Here we have various subsequent methods for things such as calculating distance between coordinates, getting the opposite door for moving between rooms, 
    getting the exterior walls for the room, playing the attack sound effect.
